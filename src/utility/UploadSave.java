...
public class UploadSave extends HttpServletRequestWrapper {
  ...
  public static String saveFile(FileItem fileItem, String upDir) {
    ...
    // 파일 태그
    System.out.println("전송된 파일명: " + fileItem.getName()); // 실제 파일 경로(전체를 포함)
 
    // 폴더 구분자 추출
    int idx = fileItem.getName().lastIndexOf("\\"); // 윈도우 기반 , 뒤에서부터 \\을 찾아서 파일명을 서브스트링으로 뽑을수있음
 
    if (idx == -1) { // 유닉스, 리눅스 기반 , 내가 찾는 게 없을경우 -1을 리턴
      idx = fileItem.getName().lastIndexOf("/");
    }
 
    // 순수 파일명 추출
    filename = fileItem.getName().substring(idx + 1); // 찾을 파일명 + 1해서 다 뽑아옴
    ...
    try {
      ...
      if (uploadedFile.exists() == true) {
        for (int k = 0; true; k++) { // 무한루프를 돌면서 찾음
          // 파일명 중복을 피하기 위한 일련 번호를 생성하여
          // 파일명으로 조합
          uploadedFile = new File(upDir, "(" + k + ")" + filename); // 리네임처리함
      ...
      // storage 폴더에 파일명 저장
      fileItem.write(uploadedFile); // 뽑아낸 내용을 storage에 저장
    } catch (Exception e) {
   ...
   * @return
   */
  // synchronized가 붙은 메소드는 한번에 하나의 스레드만 접근이 가능하며, 메소드가 사용중일 때 다른 스레드가 메소드를 호출하면 앞의 스레드가 종료될때까지 기다려야 한다.
